...
A proof generated by veriT is an indexed list of steps.
To mimic the actual proof text generated by
veriT we will write a step
\begin{plListEq}
...
in Section~\ref{sec:coreconcepts}.

The proofs can utilize Hilbert's choice operator $\epsilon$. Choice acts
...
possible otherwise.  Every proof ends with a step that has the empty
clause as the conclusion and an empty context. Section~\ref{sec:rules}
provides an overview of all proof rules used by veriT.

\section{Core Concepts of the Proof Format}
\label{sec:coreconcepts}

\todo[inline]{Move step here, introduce notation for subproofs, note that
there can be abritrary steps between premise and subproof, find good
...
\varphi_2, \dots, \varphi_n$.\todo{Update!}
...
In addition to the explicit steps, veriT might reorder equalities,
i.e. apply symmetry of the equality predicate, without generating
steps. When this happens is somewhat restricted. Equalities are only
reordered when the term below the equality change during proof search. One
such case is the instantiation of universally quantified variables. If
the variable that appears below an equality, then the equality might
have an arbitrary order after the variable is instantiated.\todo{Cleanup!}

\section{The Concrete Semantic}
...
The following lists all rules produced by veriT.
