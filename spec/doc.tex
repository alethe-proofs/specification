...
To express bitblasting in Alethe proof rules, the Alethe calculus uses
multiple families of helper functions: $\lsymb{bbterm}$, $\lsymb{bit\_of}_m$,
$\lsymb{bvsize}$, and $\lsymb{bv}_n^i$.  Functions in the families are
distinguished either by overloading ($\lsymb{bbterm}$ and $\lsymb{bvsize}$)
or by explicit indexing ($\lsymb{bit\_of}_m$ and $\lsymb{bv}_n^i$).
To avoid name clashes with user defined
functions, $\lsymb{bbterm}$ is written as \inlineAlethe{@bbterm}, $\lsymb{bit\_of}$
as \inlineAlethe{@bit_of}, $\lsymb{bvsize}$ as \inlineAlethe{@bvsize}, and
...
The family $\lsymb{bbterm}$ contains one function for each bitvector sort
$(\lsymb{BitVec}\;n)$.
\[
\lsymb{bbterm} \, : \,\underbrace{\lsymb{Bool}\, \dots\, \lsymb{Bool}}_n \; \to \; (\lsymb{BitVec} \; n).
\]

\noindent
Intuitively, the function $\lsymb{bbterm}$ takes a list of boolean arguments and
packs them into a bitvector.
Let $\langle u_0, \dots, u_{n - 1}\rangle$ denote a bitvector of sort 
$(\lsymb{BitVec}\;n)$, where $u_i = \top$ if the bit at position $i$ is $1$, and 
$u_i = \bot$ otherwise. The bit $u_{n - 1}$ is the least significant bit. This notation, 
which allows us to describe bitvectors in terms of sequences of booleans, will 
be useful when defining the several bitvector related functions. Note that:

\[
\lsymb{bbterm}\; v_0 \dots v_{n - 1} = \langle v_0, \dots, v_{n - 1}\rangle .
\]

\noindent
Also, note that the $\lsymb{bbterm}$ functions could be defined in terms of standard 
SMT-LIB functions.
\begin{align*}
\lsymb{bbterm}\;v_0 \dots v_{n - 1} :=\;
...
\noindent
The function $\lsymb{bit\_of}_m$ yields the Boolean corresponding to the $m$-th
...
$0 \leq m < n$, the concrete syntax expression  \inlineAlethe{(_ @bit_of} $m$ 
\inlineAlethe{)} will denote functions:
\[
\lsymb{bit\_of}_m : (\lsymb{BitVec}\;n) \to \lsymb{Bool}.
\]
These functions are defined as:
\[
\lsymb{bit\_of}_m \langle u_0, \dots, u_{n - 1} \rangle := u_m.
\]


\noindent
The functions $\lsymb{bvsize}$ return the size of a bitvector. Formally, there
is one $\lsymb{bvsize}$ for each bitvector sort $(\lsymb{BitVec}\;n)$.  Each
$\lsymb{bvsize}$ is a constant function that returns the size of the corresponding
bitvector sort. Using the previously introduced notation:

\begin{align*}
\lsymb{bvsize}& : (\lsymb{BitVec}\;n) \to \mathbb{N}\\
\lsymb{bvsize}&\;\langle u_0, \dots, u_{n -1} \rangle := n
\end{align*}

\noindent
Finally, $\lsymb{bv}_n^i$ is a family of constants indexed by two parameters:
a bitvector length $n$, and a natural number $i$.  We
write \inlineAlethe{(_ @bv}$n$ $i$ \inlineAlethe{)} for $\lsymb{bv}_n^i$, where
