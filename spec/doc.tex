...
\noindent
The function $\lsymb{bit\_of}_m$ extracts as a boolean bit $m$-th from a given bitvector. 
Just as the built in $\lsymb{extract}$
symbol, $\lsymb{bit\_of}_m$ is used as an indexed symbol.  Hence, for $m \leq n$, we
write \inlineAlethe{(_ @bit_of} $m$ \inlineAlethe{)} to denote functions:
\[
