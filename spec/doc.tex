...
a term like \inlineAlethe{(and a b c)} can be desuggared to \inlineAlethe{(and (and a b) c)}.

However, most SMT solvers do not handle these operators by desuggaring.
Instead, they support variadic operators naively.
To take this into account, all rules of Alethe operate on {\smtlib} terms
as they are before desuggaring.
Consider, for example, the rule \proofRule{and}. This rule implements
...
\noindent
If Alethe rules would operate on terms after desuggaring, this step would instead
...
consumers that desuggar terms as suggested by {\smtlib}, because
the two input terms \inlineAlethe{(and a b c)} and \inlineAlethe{(and (and a b) c)}
cannot be distinguished after desuggaring.
One way to work around this is to desuggar these terms like lists in functional
...
For example, the term \inlineAlethe{(and a b c)} would desuggar to
\inlineAlethe{(and (and (and a b) c) true)} and the term
\inlineAlethe{(and (and a b) c)} would desuggar to
