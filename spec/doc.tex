...
% minted 3 changed how pygmentize is called to no longer --shell-escape.
% However, since minted 3 is not widely available, we fall back to minted 2
% as the default.
...
\usepackage{environ} % Gives us NewEnviron to define environments with tables
...
% section levels to the rule index and the overall index
...
% Should we add that x_₁, …, x_n are pairwise distinct, for the notation
% $[x₁ ↦ t₁, …, x_n ↦ t_n]$ to make sense?
We will use $θ$ to denote a substitution\index{substitution}.
The notation $[x₁ ↦ t₁, …, x_n ↦ t_n]$, provided variables
$x₁, …, x_n$ are pairwise distinct, denotes the substitution
...
free variables. The application of a substitution $θ$ to a term $t$ (i.e., $θ(t)$)
...
%
The notation $t[u_1, \dots, u_n]$ stands for a term that may contain the
...
the selected {\smtlib} theory/logic as well as on those defined by the user.
However, some sorts are always available.
This includes a distinguished $\lsymb{Bool}$ sort, and
the two sorts $\lsymb{Int}$ and $\lsymb{Real}$.
The reason for this is that values of these sorts are used in the arguments
of certain rules.
%
Alethe also extends this logic with Hilbert's choice
operator $\varepsilon$.
%
...
A subtle difference between Alethe and {\smtlib} is the handling of variadic operators.
{\smtlib} handles variadic function application purely as syntactic sugar 
for chained binary applications of annotated associative functions,
while the description of Alethe rules treats them as first-class syntax.
This is described in detail on Page~\pageref{para:variadic}.
...
\begin{AletheS}
i. & $\Gamma$ & \ctxsep & $t \ceq u$ & ($\ruleType{rule}$, …) \\
\end{AletheS}

where the term $t$\, is the original term, and $u$ is the term after
preprocessing.
%
The symbol $\ceq$ is the ordinary SMT-LIB equality (usually denoted $≈$), but
we highlight because of its special role in rules that operate modulo a context.
%
Tautologies with contexts correspond to judgments
...
3.& \spctx{$y, x \mapsto y$} & \ctxsep & $x \ceq y$  & $\proofRule{refl}$\\
4.& \spctx{$y, x \mapsto y$} & \ctxsep & $(P\,x) \ceq (P\,y)$
     & $(\proofRule{cong}\,3) $\\
\spsep
...
The SMT solver {\cvcfive} is more strict.  It never applies reordering of
...
%
The format broadly follows the surface syntax specified by {\smtlib}.
%
...
& & -\grNT{numeral},\text{ or} -\grNT{decimal}.\\
 ...
 \grNT{step\_arg}            &\grRule & \grNT{symbol}\;\grOr\;\\
                                     && \textAlethe{(}\; \grNT{symbol}\; \grNT{proof\_term}\,\textAlethe{)}\;\grOr\; \\
                                     && \textAlethe{(@list} \; \grNT{proof\_term}^{+} \textAlethe{)}\\
...
\paragraph{Variadic Operators}\label{para:variadic}
{\smtlib} allows certain operators, like \inlineAlethe{and}, to be used with
a variable number of arguments.  {\smtlib} achieves this by handling applications
of more than two arguments as syntactic sugar.
%
For example, the operator \inlineAlethe{and} is annotated in {\smtlib}
with the \inlineAlethe{:left-assoc} keyword.  This has the effect that
a term like \inlineAlethe{(and a b c)} can be desugared to \inlineAlethe{(and (and a b) c)}.
%
In other words, after parsing an {\smtlib} S-expression into an abstract
syntax tree, the variadic application of these applications is broken up
into binary applications before type checking.

However, most SMT solvers do not handle these operators this way.
Instead, their internal abstract syntax tree maintains the variadic application.
%
To take this into account, all rules of Alethe operate on {\smtlib} terms
before variadic applications are broken into binary applications.
%
Consider, for example, the rule \proofRule{and}.
%
This rule implements
\inlineAlethe{and}-elimination, and takes the index of the conjunct to extract
as an argument.  The following is a correct application of the \proofRule{and}
rule:
\begin{AletheVerb}
(assume h1 (and a b c))
(step t1 (cl b) :rule and :premises h1 :args (2))
\end{AletheVerb}

\noindent
If Alethe rules would operate on terms after desugaring, this step would instead
conclude with \inlineAlethe{c}.

In the abstract notation of this document, we write these operators using
infix notation, and we add parentheses to distinguish nested applications.
For example, the following are two correct applications of the \proofRule{and} rule.

\begin{AletheX}
$i$. & \ctxsep & $a \land b \land c$ & ($\dots$) \\
$j$. & \ctxsep & $b$ & (\proofRule{and}\;$i$)\, 2\\
\end{AletheX}
...
\begin{AletheX}
$i$. & \ctxsep & $(a \land b) \land c$ & ($\dots$) \\
$j$. & \ctxsep & $c$ & (\proofRule{and}\;$i$)\, 2\\
\end{AletheX}

A consequence of this behavior is that Alethe proofs cannot be handled by
consumers that transform terms as suggested by {\smtlib}, because
the two input terms \inlineAlethe{(and a b c)} and \inlineAlethe{(and (and a b) c)}
cannot be distinguished after desugaring.
One way to work around this is to desugar these terms like lists in functional
programming languages.  The operator takes the role of the list-constructor,
and the neutral element of the operator takes the role of the empty list.
For example, the term \inlineAlethe{(and a b c)} would desugar to
\inlineAlethe{(and (and (and a b) c) true)} and the term
\inlineAlethe{(and (and a b) c)} would desugar to
\inlineAlethe{(and (and (and (and a b) true) c) true)}.
...
\tikzset{
     ...
     print/.style={draw, thin, rounded corners},
}
...
\begin{tikzpicture}[node distance=2cm, auto,>=latex', thick,scale=0.8]
    ...
    \node[print, right=0.85cm of unsat, yshift=0.8cm] (open) {\inlineAlethe{(}};
    \path[->] (unsat) edge[out=30, in=180] node[font=\scriptsize, xshift=0.6cm, yshift=0.02cm] {\texttt{get-proof}} (open);
    \path[->] (open)  edge[out=0, in=150] node[font=\scriptsize] {} (assume);
    ...
    \node[print, below=0.8cm of assume] (close) {\inlineAlethe{)}};

    \path[->] (step) edge[above, out=240, in=0] node[font=\scriptsize, xshift=-0.8cm, yshift=-0.15cm] {\textsf{Last Step}} (close);
    \path[->] (close) edge[above, out=180, in=330] node[font=\scriptsize] {} (unsat);
\end{tikzpicture}
...
is unsatisfiable.
%
The {\smtlib} problem script then requests the proof by
invoking the \inlineAlethe{get-proof} command.
%
It is possible that this command fails.
%
For example, if proof production was not activated.
%
If there is no error, the proof is printed.
Since SMT-LIB requires all response to be S-expressions, the solver first
prints an opening parenthesis '\inlineAlethe{(}'.
%
The solver then follows the behavior specified by the state machine to print the proof
as a list of Alethe commands.
%
After printing is done, the solver prints an additional
closing parenthesis '\inlineAlethe{)}'.
%
Note that these extra parentheses are printed to ensure that the proof output
is SMT-LIB compliant.
They are not part of the proof, and should only be printed when
interacting with the solver through the SMT-LIB interface.
For example, if a solver can write proofs directly into a file, it should not
write these extra parentheses.

The printing state machine ensures that a constraint is maintained:
...
\begin{AletheS}
i. & $\Gamma$ & \ctxsep & $t \ceq u$ & $(\ruleType{rule}\; \bar{p}_n)\;[\bar{a}_m]$ \\
\end{AletheS}
...
$\vDash \Gamma\,\vartriangleright\,t \ceq u$.  With the encoding described above
...
& $x + 0 \ceq y$ & $\proofRule{sum_simplify}$ \\
\end{AletheS}
...
The rule \proofRule{forall_inst} expresses quantifier instantiation.
...
The argument of a \proofRule{forall_inst} step, for a quantification of the form $\forall x_1,\dots,x_n. \varphi$,  is the list $t_1,\dots, t_n$, where $t_i$ denotes the substitute of variable $x_i$. While this information can be recovered from the term,
...
& $\varphi \ceq \psi$ & $(\dots)$ \\
\spsep
j. & $\Gamma$ & \ctxsep & $(\exists x.\,\varphi) \ceq \psi$ & $(\proofRule{sko_ex})$ \\
\end{AletheS}
...
To express bitblasting in Alethe proof rules, the Alethe calculus uses multiple 
families of helper functions: \bbterm, \bitof{m}, \bvsize, and \bv{n}{i}.  
Functions in the families are distinguished either by overloading (\bbterm{} and
\bvsize) or by explicit indexing (\bitof{m} and \bv{n}{i}). In concrete syntax,
to avoid name clashes with user defined functions, \bbterm{} is written as 
\cbbterm, \bitof{} as \cbitof{}, \bvsize{} as \cbvsize, and \bv{}{} as \cbv. The 
SMT-LIB standard specifies that simple symbols starting with ``\inlineAlethe{@}''
are reserved for solver generated functions.

The family \bbterm{} contains one function for each bitvector sort \bitvec{n}.
\[
\bbterm \, : \,\underbrace{\lsymb{Bool}\, \dots\, \lsymb{Bool}}_n \; \to \; \bitvec{n}.
\]

\noindent
Intuitively, the function \bbterm{} takes a list of boolean arguments and packs 
them into a bitvector.
Let $\langle u_0, \dots, u_{n - 1}\rangle$ denote a bitvector of sort \bitvec{n}, 
where $u_i = \top$ if the bit at position $i$ is $1$, and $u_i = \bot$ otherwise. 
The bit $u_{n - 1}$ is the least significant bit. This notation, which allows us to 
describe bitvectors in terms of sequences of booleans, will 
be useful when defining the several bitvector related functions. Note that:

\[
\bbterm\; v_0 \dots v_{n - 1} = \langle v_0, \dots, v_{n - 1}\rangle .
\]

\noindent
Also, note that the \bbterm{} functions could be defined in terms of standard 
SMT-LIB functions.
\begin{align*}
\bbterm\;v_0 \dots v_{n - 1} :=\;
 &\lsymb{concat}\,(\lsymb{concat}\,(\dots \\
 &(\lsymb{concat}\,(\lsymb{ite}\,v_0\,\langle\top\rangle\,\langle\bot\rangle)\,
                                    (\lsymb{ite}\,v_1\,\langle\top\rangle\,\langle\bot\rangle))\\
 & \dots \\
 & (\lsymb{ite}\,v_{n - 2}\,\langle\top\rangle\,\langle\bot\rangle)) \\
 & (\lsymb{ite}\,v_{n - 1}\,\langle\top\rangle\,\langle\bot\rangle)) \\
\end{align*}

\noindent
The function \bitof{m} yields the Boolean corresponding to the $m$-th
bit from a given bitvector. It is used as a symbol indexed by the desired $m$-th 
position, similarly to the built-in function $\lsymb{extract}$. Hence, for 
$0 \leq m < n$, the concrete syntax expression 
\inlineAlethe{(_ } \cbitof{} $m$\inlineAlethe{)} will denote functions:
\[
\bitof{m}: \bitvec{n} \to \lsymb{Bool}.
\]
These functions are defined as:
\[
\bitof{m} \langle u_0, \dots, u_{n - 1} \rangle := u_m.
\]


\noindent
The functions \bvsize{} return the size of a bitvector. Formally, there is one 
\bvsize{} for each bitvector sort \bitvec{n}.  Each \bvsize{}is a constant 
function that returns the size of the corresponding bitvector sort. Using the 
previously introduced notation:

\begin{align*}
\bvsize{}& : \bitvec{n} \to \mathbb{N}\\
\bvsize{}&\;\langle u_0, \dots, u_{n -1} \rangle := n
\end{align*}

\noindent
Finally, \bv{n}{i} is a family of constants indexed by two parameters:
a bitvector length $n$, and a natural number $i$.  We
write \inlineAlethe{(_} \cbv$n$ $i$ \inlineAlethe{)} for \bv{n}{i}, where
the space before $n$ (in the concrete-syntax term) is omitted for historical reasons.
Each \bv{n}{i} is the bitvector constant that represents the bitvector
...
\subsection{RARE rules}
\label{sec:alethe:rare_rules}
\emph{RARE} is a domain-specific language that allows for the definition 
of rewrite rules. There are 2 versions of the language, RARE 1.0
and RARE 2.0. The Alethe standard allows for steps in proof certificates to refer 
to rewrite rules specified in terms of the RARE 2.0 language (see
\inlineAlethe{rare_rewrite}, rule \ref{rule:rare_rewrite}, and 
\inlineAlethe{multi_rare_rewrite}, rule \ref{rule:multi_rare_rewrite}). In turn, 
RARE 2.0 is defined as a subset of the features of the so-called 
\emph{Eunoia} language: a language used to meta-program the \emph{Ethos}
proofs checker. For details about the RARE 2.0 rewriting rules specification
language, Eunoia and Ethos, see \url{https://github.com/alethe-proofs/rare2-rules/}.
Rules \ref{rule:rare_rewrite} and \ref{rule:multi_rare_rewrite}, in
section \ref{apx:rules}, introduce generic interfaces to be able to refer to 
arbitrary rewriting rules specified with RARE 2.0.
