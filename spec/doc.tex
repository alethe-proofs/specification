%
% This document is licensed under Creative Commons 4.0 Attribution International
%
...
\inlineAlethe{(+ <T1> <T2> ...)} or just \inlineAlethe{<T1>} if there is only one term,
and each term is \inlineAlethe{(* a_i <L1>)}, with a coefficient and a literal.
...
$\lsymb{pbbterm}$, $\lsymb{int\_of}_m$, $\lsymb{bvsize}$, and $\lsymb{bv}_n^i$.
...
The family $\lsymb{pbbterm}$ consists of one function for each bitvector sort
$(\lsymb{BitVec}\;n)$.
\[
    \lsymb{pbbterm}\,:\,\underbrace{\lsymb{Int}\,\dots\,\lsymb{Int}}_n\;(\lsymb{BitVec}\;n).
\]

\noindent
The function $\lsymb{pbbterm}$ takes a list of pseudo-booleans i.e. integers that must be
\inlineAlethe{0} or \inlineAlethe{1} arguments and packs them into a bitvector.

The functions $\lsymb{int\_of}_m$ are the inverse of $\lsymb{pbbterm}$.  They extract
a bit of a bitvector as a boolean.  Just as the built in $\lsymb{extract}$
symbol, $\lsymb{int\_of}_m$ is used as an indexed symbol.  Hence, for $m \leq n$, we
write \inlineAlethe{(_ @int_of} $m$ \inlineAlethe{)}, to denote functions
\[
    \lsymb{int\_of}_m : (\lsymb{BitVec}\;n) \to \lsymb{Int}.
\]
These functions are defined as
\[
    \lsymb{int\_of}_m \langle u_1, \dots, u_n \rangle := u_m.
\]
