...
The bit $u_n$ is the least significant bit. This notation, which allows us to describe bit-vectors
in terms of sequences of booleans, will be useful when defining the several 
bitvector related functions. Note that:

\[
\lsymb{bbterm}\; v_1 \dots v_n = \langle v_1, \dots, v_n\rangle .
\]
...
\noindent
The function $\lsymb{bit\_of}_m$ yields the Boolean corresponding to the $m$-th bit from a given bitvector. It is used as a symbol indexed by the desired $m$-th position, similarly to the built-in function $\lsymb{extract}$. Hence, for $m \leq n$, the concrete syntax expression  \inlineAlethe{(_ @bit_of} $m$ \inlineAlethe{)} will denote functions:
\[
\lsymb{bit\_of}_m : (\lsymb{BitVec}\;n) \to \lsymb{Bool}.
\]
These functions are defined as:
\[
\lsymb{bit\_of}_m \langle u_1, \dots, u_n \rangle := u_m.
\]
...
\lsymb{bvsize}&\;\langle u_1, \dots, u_n \rangle := n
\end{align*}
