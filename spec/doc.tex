...
& & -\grNT{numeral},\text{ or} -\grNT{decimal}.\\
 ...
 \grNT{step\_arg}            &\grRule & \grNT{proof\_term}\grOr\;
                                     \textAlethe{(@list} \;
                                        \grNT{proof\_term}^{+}
                                        \textAlethe{)}\grOr\; \textAlethe{@list}\\
...
\emph{RARE} is a domain-specific language for defining rewrite rules.
%
There are 2 versions of the language, RARE 1.0 and RARE 2.0. The Alethe standard
allows for steps in proof certificates to refer to rewrite rules specified in
terms of the RARE 2.0 language (see \inlineAlethe{rare_rewrite}, rule
\ref{rule:rare_rewrite}, and \inlineAlethe{multi_rare_rewrite}, rule
\ref{rule:multi_rare_rewrite}). In turn, RARE 2.0 is defined as a fragment of
the \emph{Eunoia} logical framework.
%
For details about the RARE 2.0 rewriting rules specification language and
Eunoia, see \url{https://github.com/alethe-proofs/rare2-rules/}.
%
...
arbitrary rewrite rules specified with RARE 2.0.
