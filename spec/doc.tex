...
by resolution. This is combined with an elegant way to reason about
...
interested in the proofs produced by veriT for a few month.  We now
...
veriT~\cite{verit}. If requested by the user, veriT outputs a proof if it
can deduce that the input problem is unsatisfiable. In proof production
mode, veriT supports the theory of uninterpreted functions, the theory of
linear integer and real arithmetic, and quantifiers.  The {\formatName}
proofs can be reconstructed by the \texttt{smt} tactic of the proof
assistant Isabelle/HOL~\cite{fleury-2019,schurr-2021}.  Furthermore,
in the future the SMT solver CVC4~\cite{CVC4} will support {\formatName}
as one of multiple proof output formats.
...
At the end of a document Section~\ref{sec:rules} contains a full list
of all proof rules used by SMT solvers using {\formatName}.
...
depend on the selected SMT-LIB theory and can also be extended by the
...
{\formatName} also uses Hilbert's choice operator $\epsilon$. Choice acts
...
\begin{plListEq}
	c_1,\,\dots,\, c_k &\proofsep i. & \varphi_1,\dots ,\varphi_l &
		(\proofRule{rule};\: p_1,\,\dots,\, p_m;\: a_1,\,\dots,\,a_n)
\end{plListEq}
\noindent
Each step is indexed by \(i \in \mathbb{N}\) and has a clause \(\varphi_1,
\dots ,\varphi_l\) has its conclusion. If a step has the empty clause
...
\dots, p_n\}$ with $p_i\in\mathbb{N}$, and a rule-dependent and possibly
...
clausal normal-form, usual disjunction is also usedhttps://bugs.launchpad.net/ubuntu/+source/unity/+bug/1463112. The usage of
...
The format uses the notion of a \emph{context} to reason about bound variables.
...
(in)equalities. Checking the validity of this formula amounts to
...
In addition to the explicit steps, veriT might reorder equalities, i.e.,
apply symmetry of the equality predicate, without generating steps. When
this happens is somewhat restricted: equalities are only reordered
...
\subsection{Inference System}  % or Rules, or Rules of Inference
...
The following lists all rules produced by veriT (Sect.~{sec:standard-rules}) and
one experimental rule that is not yet properly defined
(Sect~\ref{sec:exp-rules}).


\subsection{Main Rules}
\label{sec:main-rules}
The following rules are the raison-d'\^etre of the proof format: providing
detailed reasoning under quantifiers.
...
\subsection{Skolemization Rules}
\label{sec:skolem-rules}
...
\subsection{Resolution Rules and Clause Simplifications}
\label{sec:resolution-rules}
...
\subsection{Equality and Congruence Reasonning}
\label{sec:equality-rules}
...
\paragraph{Remark.} The trans rules comes in three flavors that can be distinguished by the attribute:
\begin{description}
\item[ordered and oriented] the equalities are given in the correct order and are oriented in the right direction;
\item[ordered and unoriented] the equalities are given in the correct order, but
  the equalitis like ``\(t_1\simeq t_2\)'' can be used as ``\(t_1\simeq t_2\)''
  or ``\(t_2\simeq t_1\)'';
...
\subsection{Clausification of Quantifiers}
...
\subsection{Clausification of Boolean Operators}
\label{sec:clausification-bools}
...
\label{sec:simplification-rules}

The following rules are pure simplification rules. We give a very detailed list.
Combining several simplifications at once is possible, but we encourage proofs
to do so for only one single theory at a time.



\subsubsection{Boolean Operators}
\label{sec:bool-simp-rules}
