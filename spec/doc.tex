...
applicability. In particular the \Taut rule is very hard to check und is
specialized.
...
The following lists all rules produced by veriT. We have sorted them by type:


\begin{description}
\item[Main Rules (Sect.~\ref{sec:main-rules})] contains the rules that justify
  the existence of the proof format, namely the \proofRule{bind} and
  \proofRule{Subproof} rules.
\item[Skolemization Rules (Sect.~\ref{sec:skolem-rules})] that uses Hilbert choice.
\item[Basic Proof Rules (Sect.~\ref{sec:basic-rules}).]
\item[Arithmetic Rules (Sect.~\ref{sec:arith-rules}).]
\item[Quantifier Rules (Sect.~\ref{sec:quantifier-rules})] that shows how to instantiate a formula.
\item[SAT handling (Sect.~\ref{sec:resolution-rules})] where resolution and
  removal of duplicated literals are presented.
\item[Clausification of] Boolean operators
  (Sect.~\ref{sec:clausification-bools}) and IfThenElse
  (Sect.~\ref{sec:ite-rules}) where structure is moved from terms to the clause
  structure.
\item[Simplifications (Sect.~\ref{sec:simplification-rules})] are a very
  important part of any SMT solver. veriT provides very detailed rules. To
  simplify checking, we encourage users to write one step per theory instead of
  combining all simplifications in a single step.
\item[Term Reshaping (Sect.~\ref{sec:unsorted-rules})] that transform a term by
...
clause. Take the clause with the double negation and resolve on \(\varphi\).
...
\subsection{Clausification of If-Then-Else}
\label{sec:ite-rules}
