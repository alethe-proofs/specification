...
\newcommand{\ruleref}[1]{\autoref{rule:#1}:~\proofRule{\nameref{rule:#1}}}
...
SMT-LIB format.
% PF elegant: this is self-congratulation.  Try to find a more objective algorithm: unified, clean, simple,...
...
possible otherwise.
% PF TODO \epsilon vs varepsilon
% PF it might be useful to say that \epsilon is functional, i.e., if \varphi is logically equivalent to \varpsi then \epsilon gives the same result
...
\dots, p_n\}$ with $p_i\in\mathbb{N}$,
% PF TODO really, N?  not indices?
...
% PF TODO aren't you confusing clauses and disjunctions above?
...
% PF TODO: check this sentence, something is wrong
...
In the next section we provide a list of all proof rules supported by
...
Rule & Description \\
\hline
\ruleref{assume} & Repeats an input assumption. \\
\ruleref{subproof} & Concludes a subproof with assumptions. \\
\end{tabular}
\end{table}
...
\begin{table}[h!]
\caption{Resolution and Clause Simplification}
\centering
\begin{tabular}{l c}
Rule & Description \\
\hline
...
\begin{table}[h!]
\caption{Recommend Rules.  Some rules are very similar.  This table lists
...
The following lists all rules of {\formatName}. They are categorized by type:
...
\item[Clausification of] Boolean operators
  (Sect.~\ref{sec:clausification-bools}) and if-then-else
