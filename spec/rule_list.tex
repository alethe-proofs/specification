...
\begin{RuleDescription}{pbblast_bveq}
    ...
    \[
        \begin{array}{lcl}
            x_i & = & (\lsymb{intOf}_{i}\ \textbf{x}) \\
            y_i & = & (\lsymb{intOf}_{i}\ \textbf{y})
        \end{array}
    \]
    Alternatively, the rule may also be phrased as a ``short-circuiting''
    of the above when \textbf{x} and \textbf{y} are ``$\lsymb{pbbT}$'' applications. So given that
    \[
        \begin{array}{lcl}
            \textbf{x} & = & (\lsymb{pbbT}\ x_0\ \ldots\ x_i\ \ldots \ x_j\ \ldots\ x_n) \\
            \textbf{y} & = & (\lsymb{pbbT}\ y_0\ \ldots\ y_i\ \ldots \ y_j\ \ldots\ y_n) \\
        \end{array}
    \]
    then ``$x_{i}$'' and ``$y_{i}$'' can be the arguments of ``$\lsymb{pbbT}$'' directly.
...
\begin{RuleDescription}{pbblast_pbbvar}
    ...
    \begin{AletheS}
        $i$. & \ctxsep & \, & $\textbf{x} \approx
            \lsymb{pbbT}\; x_0 \dots x_{n-1}$ & (\currule)
    \end{AletheS}
...
\begin{RuleDescription}{pbblast_pbbconst}
    ...
    \begin{AletheS}
        $i$. & \ctxsep & \, & $b \approx
            \lsymb{pbbT}\; \textbf{match}(b_0) \dots \textbf{match}(b_{n-1}) $ & (\currule)
    \end{AletheS}

    \noindent
    In which we expand \textbf{match($b_i$)} into the constant value of ``$(\lsymb{intOf}_{i}\ b)$''
\end{RuleDescription}
...
\begin{RuleDescription}{pbblast_bvxor}
    ...
    \begin{AletheS}
        $i$. & \ctxsep & \, & $(\lsymb{bvxor}\ \textbf{x}\ \textbf{y}) \approx
            \lsymb{pbbT}\; c_0 \dots c_{n-1} $ & (\currule)
    \end{AletheS}
