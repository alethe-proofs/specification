...
\begin{RuleDescription}{pbblast_bvxnor}
    % TODO: Reframe the rule, not in function of bvnot and bvxor
    The `bvxnor' operation over BitVectors \textbf{x} and \textbf{y}
    with $n$ bits is expressed using PseudoBoolean inequalities by the negation
    of the `bvxor` operator, so

    \begin{AletheS}
        $i$. & \ctxsep & \, & $(\lsymb{bvxnor}\ \textbf{x}\ \textbf{y}) \approx \lsymb{bvnot} (\lsymb{bvxor}\ \textbf{x}\ \textbf{y})$ & (\currule)
    \end{AletheS}
