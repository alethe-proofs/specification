...
$i$. & \ctxsep & $ \forall x_1, \dots, x_n.(\varphi_1 \lor \cdots \lor \varphi_m) ≈
  (\forall x_{1,1}, \dots, x_{1,n}. \varphi_1) \lor \cdots \lor
  (\forall x_{1,m}, \dots, x_{m,n}. \varphi_m) $ & (\currule) \\
\end{AletheX}
...
$i$. & \ctxsep & $ \exists x_1, \dots, x_n.(\varphi_1 \land \cdots \land \varphi_m) ≈
  (\exists x_{1,1}, \dots, x_{1,n}. \varphi_1) \land \cdots \land
  (\exists x_{1,m}, \dots, x_{m,n}. \varphi_m) $ & (\currule) \\
\end{AletheX}

where $\{x_{1,1}, \dots, x_{m,n}\}\subseteq\{x_1, \dots, x_n\}$
...
\begin{RuleDescription}{arith_poly_norm}
\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule) \\
\end{AletheX}

This method normalizes arithmetic expressions $t$ and $s$, such that expressions
representing the same value are identified.

\textbf{TODO}: should we extend $≈$ with every pair that is arith\_poly\_norm?

\textbf{TODO}: should we consider $arith\_poly\_norm \; t ≈ arith\_poly\_norm \; s$ 
for some function $arith\_poly\_norm$?

\textbf{TODO}: should we force some particular normalization?
\end{RuleDescription}
