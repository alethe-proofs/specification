...
\begin{AletheXS}
$i_1$. & & \ctxsep & $t_1 \ceq t_1'$ & ($\dots$) \\
$i_2$. & & \ctxsep & $t_2 \ceq t_2' $ & ($\dots$) \\
\aletheLineS
$i_n$. & & \ctxsep & $t_n \ceq t_n' $ & ($\dots$) \\
$j$.   & & \ctxsep & $t \ceq s$
    & (\currule\; $i_1$, $\dots$, $i_n$) [$\lsymb{rule\_id},\; a_1,\; \dots,\;a_m$] \\
\end{AletheXS}

This rule refers to the application of the RARE 2.0 rewrite identified by the
string literal
$\lsymb{rule\_id}$ and which justifies concluding $t ≈ s$..
%
As described
in Section \ref{sec:alethe:rare_rules}, a RARE rewrite rule is defined using a
domain-specific language, called RARE.
%
The \currule{} step is correct when its premises $i_1,\dots,i_n$ match the
conditions established in the specification of the RARE rewrite, if any, and its
conclusion matches the conclusion the conclusion established in the
specification instantiated with the given arguments $a_1\dots,a_m$, if any.

An important detail is that RARE admits a list semantics for associative
operators, so some $a_k$ may correspond to a list of terms, which is represented
as $(\lsymb{@list} \; a_{k,1}, \dots, a_{k,l})$.
