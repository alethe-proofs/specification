...
\end{AletheX}
where $\varphi_i$ is either $\neg l_i$ or $l_i$, but it never is an equality of the form $s_1 ≈ s_2$, for some arbitrary term $s_1$ and $s_2$. However, the rules allows negated equalities: $\neg (s_1 ≈ s_2)$.
...
\item
 ...
 \item The resulting inequality has the form:
  ...
  \item \label{la_generic:str}
    ...
    \item Multiply both sides of the inequality by its corresponding coefficient 
...
added, and $d'^k$ is its right-hand side. The relation $\bowtie_{sum}$
is the ``weakest'' relation among the ones that appear in the involved 
inequalities added. That is:
\begin{itemize}
\item $\bowtie'$ is some relation $\bowtie$, if all the relations involved are 
exactly $\bowtie$.
\item $\bowtie'$ is $>$ if all the relations either $≈$ or $>$
\end{itemize}
, $>$ if all are
