...
the form $s_1 ≈ s_2$, for corresponding left-hand and right-hand side terms 
$s_1$ and $s_2$. However, the rules allow negated equalities: $\neg (s_1 ≈ s_2)$.

The constants $a_i$ are of sort \inlineAlethe{Real} and must be written
following the rules described by productions of the non-terminals
...
\item
 ...
 \item If $\varphi_j = \neg (s_1 \bowtie s_2)$, then replace it by
   ...
   $s_1 \bowtie s_2$ in terms of the relations
   $>$ or $\geq$, accordingly, as is done in the previous step.
 ...
 \item The resulting inequality has the form:
  ...
  \item \label{la_generic:str}
    ...
    If some variable in $s$ is not integer sorted, consider $\bowtie' = \; \bowtie$ and 
    $d' = \; d$ in the following steps.
