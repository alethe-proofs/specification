...
\begin{RuleDescription}{pbblast_bveq}
    ...
    \[
        \begin{array}{lcl}
            \mathrm{x}_i & = & (\lsymb{bitOf}_{i}\ x) \\
            \mathrm{y}_i & = & (\lsymb{bitOf}_{i}\ y)
        \end{array}
    \]
    Alternatively, the rule may also be phrased as a ``short-circuiting''
    of the above when \textbf{x} and \textbf{y} are ``$\lsymb{bbT}$'' applications. So given that
    \[
        \begin{array}{lcl}
            x & = & (\lsymb{bbT}\ x_0\ \ldots\ x_i\ \ldots \ x_j\ \ldots\ x_n) \\
            y & = & (\lsymb{bbT}\ y_0\ \ldots\ y_i\ \ldots \ y_j\ \ldots\ y_n) \\
        \end{array}
    \]
    then ``$x_{i}$'' and ``$y_{i}$'' can be the arguments of ``$\lsymb{bbT}$'' directly.
