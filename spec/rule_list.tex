...
\ruleref{miniscope_distribute} & Distribution of quantified variables over $\lor$, $\land$. \\
\ruleref{miniscope_split}      & Splitting of quantified variables over $\lor$, $\land$. \\
\ruleref{miniscope_ite}        & Miniscoping of if-then-else. \\
\end{xltabular}
...
\ruleref{bool_simplify} & Simplification of combined Boolean connectives. \\
\ruleref{ac_simp} & Flattening and removal of duplicates for $\lor$ or $\land$. \\
\ruleref{aci_simp} & Simplification of terms through ACI reasoning. \\
\ruleref{ite_simplify} & Simplification of if-then-else. \\
\ruleref{qnt_simplify} & Simplification of constant quantified formulas. \\
\ruleref{onepoint} & The one-point rule. \\
\ruleref{qnt_join} & Joining of consecutive quantifiers. \\
...
\ruleref{bitblast_neg} & Bitblasting of $\lsymb{add}$. \\
\end{xltabular}
...
This rule is the chain resolution of two or more clauses.
...
\end{AletheX}
where $l^{r_1}_{s_1} , \dots , l^{r_m}_{s_m}$ are disjuncts $l^{i}_{j}$ (for 
$1 \leq i \leq n$, $1 \leq j \leq max \{k^1, ..., k^n \}$) from the clauses in 
premises $i_1,\dots, i_n$, which result from a chain of binary resolution 
steps on the clauses of these premises. It is possible that $m = 0$, i.e. that
...
{\isabelle} shows that checking can be done by naive decision procedures. The
...
constants, $c_i$ and $t_i$ have the same sort for all $i$, 

Let $l_1,\dots, l_n$ be linear inequalities and $a_1, \dots, a_n$
rational numbers, then a {\currule} step has the form

\begin{AletheX}
$i$. & \ctxsep & $\varphi_1$, \dots , $\varphi_n$  & \currule\, [$a_1$, \dots, $a_n$] \\
\end{AletheX}
where $\varphi_i$ is either $\neg l_i$ or $l_i$, but it never is an equality of 
the form $s_1 ≈ s_2$, for terms $s_1$ and $s_2$. However, the rules allow negated 
equalities: $\neg (s_1 ≈ s_2)$.

The constants $a_i$ are of sort \inlineAlethe{Real} and must be written
following the production rules for the non-terminals
$\grNT{rational}$, $\grNT{decimal}$ or $\grNT{nonpositive\_decimal}$
(see Figure \ref{fig:grammar} in Section \ref{sec:alethe:syntax}).

To check the unsatisfiability of the negation of $\varphi_1\lor \dots
\lor \varphi_o$ one performs the following steps for each literal
$\varphi_j$ and corresponding constant $a_j$:

\begin{enumerate}
\item \label{la_generic:first_step}
  \begin{itemize}
  \item If $\varphi_j = s_1 > s_2$, then replace it by $s_2 \geq s_1$.
  \item If $\varphi_j = s_1 \geq s_2$, then replace it by $s_2 > s_1$.
  \item If $\varphi_j = s_1 < s_2$, then replace it by $s_1 \geq s_2$.
  \item If $\varphi_j = s_1 \leq s_2$, then replace it by $s_1 > s_2$. 
  \end{itemize}
    
      This negates the literal. Note that, in these cases, we always end up 
      with inequalities expressed using only $>$ or $\geq$. This simplifies the 
      description of the following steps.

 \item If $\varphi_j = \neg (s_1 \bowtie s_2)$, then replace it by
   $s_1 \bowtie s_2$. If $\bowtie$ is $<$ or $\leq$, redefine the inequality
   $s_1 \bowtie s_2$ in terms of the relations $>$ or $\geq$, accordingly:

   \begin{itemize}
   \item If $s_1 \bowtie s_2 = s_1 < s_2$, then replace it by $s_2 > s_1$.
   \item If $s_1 \bowtie s_2 = s_1 \leq s_2$, then replace it by $s_2 \geq s_1$.
   \end{itemize}

 \item The resulting inequality has the form:
   \[
     \sum_{i=0}^{n}c_i\times{}t_i +
     d_1\bowtie \sum_{i=n+1}^{m} c_i\times{}t_i + d_2
   \]
   ...
   Replace it by $s \bowtie d$,  where 
   $s = \sum_{i=0}^{n}c_i\times{}t_i - \sum_{i=n+1}^{m} c_i\times{}t_i$ and
   $d := d_2 - d_1$.

  \item \label{la_generic:str}
    If all variables in $s$ are integer sorted, replace $\bowtie$ and $d$ 
    by $\bowtie'$ and $d'$ as defined in the following
    table:

    \begin{tabular}{r l l}
      $\bowtie$  & If $d$ is an integer  & Otherwise \\
      $>$        & $\bowtie' \; = \; \geq, d' = d + 1$  & 
                   $\bowtie' \; = \; \geq, d' =  \lfloor d\rfloor + 1$  \\
      $\geq$     & $\bowtie' \; = \; \geq, d' = d$      & 
                   $\bowtie' \; = \; \geq, d' = \lfloor d\rfloor + 1$  \\
      $≈$         & $\bowtie' \; = \; ≈, d' = d$      & 
                   $\bowtie' \; = \; ≈, d' = d$        \\
    ...
    If some variable in $s$ is not integer sorted, consider 
    $\bowtie' = \; \bowtie$ and $d' = \; d$ in the following steps.

    \item \label{la_generic:final_sum}
      Multiply both sides of the inequality by its corresponding coefficient 
      $a_j$, if $\bowtie'$ is $≈$, or by $|a_j|$, otherwise. We obtain:
      \[\sum_{i=0}^{m}a_j\times{}c_i\times{}t_i ≈ a_j\times{}d'\]
      or
      \[\sum_{i=0}^{m}|a_j|\times{}c_i\times{}t_i \bowtie' |a_j|\times{}d'\]
      respectively.
\end{enumerate}

Finally, the sum of the obtained $n$ inequalities should result in a trivial 
contradiction. The sum results in an inequality of the form:
\[
    \sum_{k=1}^{n}(\sum_{i=0}^{m^k}a'_k*c_i^k*t_i^k) \bowtie_{sum} \sum_{k=1}^{n}d'^k
\]
where $\sum_{i=0}^{m^k}a'_k*c_i^k*t_i^k$ is the left-hand side of the $k$-th inequality
added (being ${a'_k}$, either, $a_k$ or $|a_k|$, according to step 
\ref{la_generic:final_sum}), and $d'^k$ is its right-hand side. Also,
$\bowtie_{sum}$ is the ``most strict'' order among the relations that appear in the
involved inequalities added, or just $≈$. That is:
\begin{itemize}
\item $\bowtie_{sum}$ is some relation $\bowtie$, if all the relations involved are 
exactly $\bowtie$.
\item $\bowtie_{sum}$ is $>$ if some of the relations involved are $>$.
\item $\bowtie_{sum}$ is $\geq$ in any other case.
\end{itemize}

The coefficients $a_1, \dots, a_n$ provided to the rule must be such that the sum 
on the left-hand side is $0$, and the right-hand side is $>0$ (or $\geq 0$ if 
$\bowtie_{sum}$ is $>$).
\end{RuleDescription}
...
This rule introduces a tautology of linear arithmetic.
The conclusion is always a unit clause.

\begin{AletheX}
$i$. & \ctxsep & $\varphi$  & (\currule) \\
\end{AletheX}

If the literal $\varphi$ is either a linear inequality $s_1 \bowtie s_2$
or $\neg(s_1 \bowtie s_2)$, it is enough to perform steps 1 to 3 of the process for
checking the \proofRule{la_generic}. The result is trivially unsatisfiable.
For example, if $\varphi$ is $\neg (n \leq n - 1)$ we get
$0 \leq -1$ after step 3.

If the literal is not a linear inequality, it is a binary clause
$\varphi_1 \lor \varphi_2$.  In this case the step is equivalent to

\begin{AletheX}
$i$. & \ctxsep & $\varphi_1$, $\varphi_2$ & \proofRule{la_tautology}\, [$1.0$, $1.0$] \\
\end{AletheX}

\ruleparagraph{Remark.}
The second form is inteded to express bounds on linear combinations. 
In the case of veriT, the conclusion has one of the following forms,
where $d_1$ and $d_2$ are decimals or integers, and $s$ is a constant.
\begin{itemize}
    \item $\neg (s \leq d_1) \lor s \leq d_2$ where $d_1 \leq d_2$
    \item $s \leq d_1 \lor \neg (s \leq d_2)$ where $d_1 = d_2$
    \item $\neg (s \geq d_1) \lor s \geq d_2$ where $d_1 \geq d_2$
    \item $s \geq d_1 \lor \neg (s \geq d_2)$ where $d_1 = d_2$
    \item $\neg (s \leq d_1) \lor \neg(s \geq d_2)$ where $d_1 < d_2$
\end{itemize}
\end{RuleDescription}
...
$j$. & \spctx{ $\Gamma, y_1,\dots, y_n,  x_1 \mapsto y_1, \dots ,  x_n \mapsto y_n$}
   & \ctxsep & $\varphi \ceq \varphi'$ & ($\dots$) \\
   \spsep
$k$. & $\Gamma$ & \ctxsep &
    $Q x_1, \dots, x_n.\varphi \ceq Q y_1, \dots, y_n. \varphi'$
     & \currule{} \\
\end{AletheXS}
...
\spctx{$\Gamma, x_1 \mapsto \varepsilon_1, \dots ,  x_n \mapsto \varepsilon_n$}
   & \ctxsep &  $\varphi \ceq \psi$ & ($\dots$) \\
   \spsep
$k$. & $\Gamma$ & \ctxsep & $\exists x_1, \dots, x_n.\varphi \ceq \psi$ & \currule{} \\
\end{AletheXS}
...
\spctx{$\Gamma, x_1 \mapsto (\varepsilon x_1.\neg\varphi), \dots,  x_n \mapsto (\varepsilon x_n.\neg\varphi)$}
   & \ctxsep & $\varphi \ceq \psi$ & ($\dots$) \\
 \spsep
$k$. & $\Gamma$ & \ctxsep  & $\forall x_1, \dots, x_n.\varphi \ceq \psi$ & \currule{} \\
\end{AletheXS}
...
\noindent
where $x_i$ and $t_i$ have the same sort, and $x_1, \dots, x_n$ are pairwise
distinct.
...
\begin{AletheXS}
$j$. & $\Gamma$ & \ctxsep & $t_1 \ceq t_2 $ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i_1$. & $\Gamma$ & \ctxsep & $t_1 \ceq t_2 $ & ($\dots$) \\
$i_2$. & $\Gamma$ & \ctxsep & $t_2 \ceq t_3 $ & ($\dots$) \\
\aletheLineS
$i_n$. & $\Gamma$ & \ctxsep & $t_n \ceq t_{n+1} $ & ($\dots$) \\
$j$.   & $\Gamma$ & \ctxsep & $t_1 \ceq t_{n+1}$ & (\currule\; $i_1$, $\dots$, $i_n$) \\
\end{AletheXS}
\end{RuleDescription}

\begin{RuleDescription}{cong}
\begin{AletheXS}
$i_1$. & $\Gamma$ & \ctxsep & $t_1 \ceq u_1$ & ($\dots$) \\
$i_2$. & $\Gamma$ & \ctxsep & $t_2 \ceq u_2 $ & ($\dots$) \\
\aletheLineS
$i_m$. & $\Gamma$ & \ctxsep & $t_m \ceq u_m $ & ($\dots$) \\
$j$.   & $\Gamma$ & \ctxsep & $(f\,s_1\,\cdots\,s_n\, t_1\,\cdots\,t_m) \ceq
                               (f\,s_1\,\cdots\,s_n\, u_1\,\cdots\,u_m)$
    & (\currule\; $i_1$, $\dots$, $i_m$) \\
\end{AletheXS}
where $f$ is any $n+m$-ary function symbol of appropriate sort.

\ruleparagraph{Remark.} Note that $n$ can be $0$.  If $n > 0$ the omitted
premises correspond to simple reflexivity steps.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & 
      $\circ\ t_1 \dots t_n \ceq \circ\ u_1 \dots u_n $ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $\varphi_1\land \cdots\land\varphi_n \ceq \psi$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(\varphi_1\lor \cdots\lor\varphi_n) \ceq \psi$ & \currule \\
\end{AletheXS}
where $\psi$ is the transformed term.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $\varphi_1\rightarrow \varphi_2 \ceq \psi$ & \currule \\
\end{AletheXS}
where $\psi$ is the transformed term.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep  & $(\varphi_1≈ \varphi_2) \ceq \psi$ & \currule \\
\end{AletheXS}
where $\psi$ is the transformed term.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $\varphi\ceq \psi$ & \currule \\
\end{AletheXS}
where $\psi$ is the transformed term.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $\psi \ceq \varphi_1 \circ\cdots\circ\varphi_n$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(\lsymb{ite}\,\varphi\,t_1\,t_2) \ceq u$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(\forall x_1, \dots, x_n. \varphi) \ceq \varphi$ & \currule \\
\end{AletheXS}
...
$j$. & \spctx{$\Gamma, x_{k_1},\dots, x_{k_m},  x_{j_1} \mapsto t_{j_1}, \dots ,  x_{j_o} \mapsto t_{j_o}$}
   & \ctxsep & $\varphi \ceq \varphi'$ & ($\dots$) \\
 \spsep
$k$. & $\Gamma$ & \ctxsep  & $Q x_1, \dots, x_n.\varphi \ceq Q x_{k_1}, \dots, x_{k_m}. \varphi'$ & \currule{} \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $Q x_1, \dots, x_n.\,(Q x_{n+1}, \dots, x_{m}.\,\varphi)
      \ceq Q x_{k_1}, \dots, x_{k_o}.\,\varphi$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $Q x_1, \dots, x_n.\,\varphi \ceq Q x_{k_1}, \dots, x_{k_m}.\,\varphi$
 & \currule \\
\end{AletheXS}
  where $m \leq n$ and $Q\in\{\forall, \exists\}$. Furthermore, $k_1, \dots, k_m$ is
  a monotonic map to $1, \dots, n$ and if $x\in \{x_j\; |\; j \in \{1, \dots,
  n\} \land j\notin \{k_1, \dots, k_m\}\}$ then either $x$ is not free in $\varphi$ or
  $x\in \{x_j\; |\; j \in \{k_1,\dots,k_m\}\}$ (i.e., duplicates can be removed).
\end{RuleDescription}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(t_1≈ t_2) \ceq \varphi$ & \currule \\
\end{AletheXS}
  where $\psi$ is the transformed term.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(t_1\, /\,  t_2) \ceq t_3$ & \currule \\
\end{AletheXS}
The possible transformations are:
\begin{itemize}
  \item $t\, /\, t ⇒ 1$
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $t_1\times\cdots\times t_n \ceq u$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $- (-t) \ceq t$ & \currule \\
\end{AletheXS}
or

\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $-t \ceq u$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $t_1- t_2 \ceq u$ & \currule \\
\end{AletheXS}
The possible transformations are:
\begin{itemize}
    \item $t - t ⇒ 0$
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $t_1+\cdots+t_n \ceq u$ & \currule \\
\end{AletheXS}
where $u$ is either a constant or a product.
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $t_1 \bowtie t_2 \ceq \psi$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i_1$. & $\Gamma$ & \ctxsep & $t_{1} \ceq s_{1}$ & ($\dots$) \\
\aletheLineS
$i_n$. & $\Gamma$ & \ctxsep & $t_{n} \ceq s_{n}$ & ($\dots$) \\
\aletheLineS
$j$. & \spctx{$\Gamma, x_1 \mapsto s_1, \dots,  x_n \mapsto s_n$}
   & \ctxsep &  $u \ceq u'$ & ($\dots$) \\
\spsep
$k$. & $\Gamma$ & \ctxsep &
     $(\lsymb{let}\,x_1 = t_1,\, \dots,\, x_n = t_n\,\lsymb{in}\, u) \ceq u'$
...
$j$. & \spctx{$\Gamma, y_1,\dots, y_n,  x_1 \mapsto y_1, \dots ,  x_n \mapsto y_n$}
   & \ctxsep &  $u \ceq u'$ & ($\dots$) \\
\spsep
$k$. & $\Gamma$ & \kern-28em\ctxsep & % The kern hacking here is to make the rule readable
      $\kern-14em \lsymb{let}\,x_1=t_1,\, \dots,\, x_n=t_n \lsymb{in}\, u \ceq
...
This rule eliminates the $\lsymb{distinct}$ predicate.

This rule behaves differently when the arguments are terms of type $\lsymb{Bool}$.
Then there are two cases:

\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep &
$(\lsymb{distinct}\,\varphi\,\psi) \ceq \neg (\varphi ≈ \psi)$ & \currule \\
\end{AletheXS}
and

\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep &
$(\lsymb{distinct}\,\varphi_1\,\varphi_2\,\varphi_3\,\dots) \ceq \bot$ & \currule \\
\end{AletheXS}

For all other sorts the rule is

\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep &
$(\lsymb{distinct}\,t_1\,\dots\, t_n) \ceq
\bigwedge_{i=1}^{n}\bigwedge_{j=i+1}^{n} t_i\;{\not≈}\;t_j$ & \currule \\
\end{AletheXS}

\ruleparagraph{Remark.}
SMT-LIB does not allow $\lsymb{distinct}$ to be used with only one argument.
However, multiple solvers support this as a non-standard extension.
While Alethe does not prescribe a behavior for this case, most solvers
produce the following step:

\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $(\lsymb{distinct}\, t) \ceq \top$ & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$ & \ctxsep & $\bigcirc_{i=1}^{n} t_i \ceq (\dots(( t_1\circ  t_2) \circ  t_3)\circ \cdots  t_n)$
 & \currule \\
\end{AletheXS}
...
\begin{AletheXS}
$i$. & $\Gamma$  & \ctxsep & $\bigcirc_{i=1}^{n} t_i \ceq
...
\begin{AletheX}
$i$. & \ctxsep & $(\bvextract{j}{i})\ x ≈ \bbterm\ \varphi_i\ \ldots\ \varphi_j$ & \currule \\
\end{AletheX}

\noindent
where $\varphi_k = \bitof{k}\ x$, for $i \leq k \leq j$.
...
\medskip
\begin{AletheX}
$i$. & \ctxsep & $(\bvextract{j}{i})\ (\bbterm\ x_0\ \ldots\
x_i\ \ldots \ x_j\ \ldots\ x_n) \; ≈ \; \bbterm\ x_i\ \ldots\ x_j$ & \currule \\
\end{AletheX}

\noindent
This alternative is based on the validity of the equality
\[
\bitof{k}\ (\bbterm\ x_0\ \ldots\ x_i\ \ldots \ x_j\ \ldots\ x_n) ≈ x_k
\]
for any bit-vector $x$ of size $n+1$, where $0\leq k\leq n$.

\end{RuleDescription}

\begin{RuleDescription}{bitblast_concat}
\begin{AletheX}
$i$. & \ctxsep & $\bvconcat{x}{y}  ≈ A_1$ & \currule \\
\end{AletheX}
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x) \, \ldots \,(\bitof{n-1}\ x) \\
              & (\bitof{0}\ y) \, \ldots \, (\bitof{m-1}\ y)
\end{align*}

Note that the vectors involved in the operation do not need to have the same width.
Alternatively, the rule may also be phrased as a ``short-circuiting'' of the
above when $x$ and $y$ are ``$\lsymb{bbterm}$'' applications. So given that:
\[
  \begin{array}{lcl}
    x&=&\bbterm\ x_0\ \ldots\ x_{n-1}\\
    y&=&\bbterm\ y_0\ \ldots\ y_{m-1}\\
  \end{array}
\]
then, the rule can be alternatively phrased as:

\begin{AletheX}
$i$. & \ctxsep & $\bvconcat{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ x_0 \,\ldots \, x_{n-1}\, y_0 \,\ldots \, y_{m-1}$

\end{RuleDescription}

\begin{RuleDescription}{bitblast_sext}
\begin{AletheX}
$i$. & \ctxsep & $\bvsext{k}{x}  ≈ A_1$ & \currule \\
\end{AletheX}
where $x$ has the type \bitvec{n}. The term ``$A_1$'' is:
\begin{align*}
\bbterm\;& (\bitof{0}\ x) \, \ldots \,(\bitof{n-1}\ x) \\
              & (\bitof{n-1}\ x) \, \ldots \, (\bitof{n-1}\ x)
\end{align*}

That is, the original vector is extended just by repeating its most significant 
bit $k$ times, such that the resulting vector has width $n + k$, and
$\bitof{n}\ (\bvsext{k}{x}) = \bitof{n-1}\ x$, ..., 
$\bitof{n + k}\ (\bvsext{k}{x}) = \bitof{n-1}\ x$.
Alternatively, the rule may also be phrased as a ``short-circuiting'' of the
above when $x$ is a ``$\lsymb{bbterm}$'' application. So given that:
\[
  \begin{array}{lcl}
    x&=&\bbterm\ x_0\ \ldots\ x_{n-1}
  \end{array}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvsext{k}{x} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ x_0 \,\ldots \, x_{n-1}\, x_{n-1} \,\ldots \, x_{n-1}$,
where, again, the most significant bit of $x$, that is, $x_{n-1}$, is repeated
$k$ times.

\end{RuleDescription}

\begin{RuleDescription}{bitblast_eq}
\begin{AletheX}
$i$. & \ctxsep & $\bveq{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
& (\bitof{0}\ x \,\leftrightarrow\, \bitof{0}\ y) \; \wedge \\
  & (\bitof{1}\ x \,\leftrightarrow\, \bitof{1}\ y) \; \wedge \\ \\
  & \ldots \\
  & (\bitof{n-1}\ x \,\leftrightarrow\, \bitof{n-1}\ y) \\
\end{align*}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvult{x}{y} ≈ \mathrm{res}_{n-1}$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n} and, for
$i\geq 0$:
\[
  \begin{array}{lcl}
    \mathrm{res}_0&=&\neg (\bitof{0}\ x) \wedge \bitof{0}\ y\\
    \mathrm{res}_{i+1}&=&((\bitof{i + 1}\ x ≈ \bitof{i + 1}\ y)\wedge \mathrm{res}_i)\vee
                       (\neg (\bitof{i+1}\ x) \wedge \bitof{i+1}\ y)
  \end{array}
\]
...
\[
  \begin{array}{lcl}
    x&=&\bbterm\ x_0\ \ldots\ x_i\ \ldots \ x_j\ \ldots\ x_n\\
    y&=&\bbterm\ y_0\ \ldots\ y_i\ \ldots \ y_j\ \ldots\ y_n\\
  \end{array}
\]
then ``$\mathrm{res}_i$'' can be defined, for $i \geq 0$, as
\[
  \begin{array}{lcl}
    \mathrm{res}_0&=&\neg x_0 \wedge y_0\\
...
\begin{RuleDescription}{bitblast_slt}

\begin{AletheX}
$i$. & \ctxsep & $\bvslt{x}{y} ≈ \mathrm{res}$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}, where:
$$ \mathrm{res} = ((\bitof{n - 1}\ x ≈ \bitof{n - 1}\ y)\wedge \mathrm{res}_{n - 2})
                   \vee
                    (\bitof{n - 1}\ x \wedge \neg (\bitof{n - 1}\ y)) $$
and, for $i\geq 0$:
\[
...
\[
  ...
  \end{array}
\]
then ``$\mathrm{res}$'' can be defined as:
$$ \mathrm{res} = ((x_{n - 1} ≈ y_{n - 1})\wedge \mathrm{res}_{n - 2})
                   \vee
                    (x_{n - 1} \wedge \neg y_{n - 1}) $$
where, for $i \geq 0$:
\[
...
\begin{AletheX}
$i$. & \ctxsep & $\bvadd{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (((\bitof{0}\ x) \,\lsymb{xor}\,(\bitof{0}\ y))\,\lsymb{xor}\,\mathrm{carry}_0) \\
              & (((\bitof{1}\ x) \,\lsymb{xor}\,(\bitof{1}\ y))\,\lsymb{xor}\,\mathrm{carry}_1) \\
              & \ldots \\
              & (((\bitof{n - 1}\ x) \,\lsymb{xor}\, (\bitof{n - 1}\ y))\,\lsymb{xor}\,\mathrm{carry}_{n-1})
\end{align*}
and, for $i\geq 0$:
\[
  \begin{array}{lcl}
    \mathrm{carry}_0&=&\bot\\
    \mathrm{carry}_{i+1}&=&((\bitof{i}\ x) \wedge \bitof{i}\ y) 
                                                    \vee 
                                                   (((\bitof{i}\ x)\,\lsymb{xor}\, (\bitof{i}\ y))\wedge \mathrm{carry}_i)
  \end{array}
\]
...
\begin{AletheX}
$i$. & \ctxsep & $\bvadd{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ ((x_0 \,\lsymb{xor}\, y_0)\,\lsymb{xor}\,\mathrm{carry}_0)\ 
                                          \ldots\ 
                                          ((x_{n-1}\,\lsymb{xor}\, y_{n-1})\,\lsymb{xor}\,\mathrm{carry}_{n-1})$, 
and ``$\mathrm{carry}_k$'' being defined, for $k \geq 0$, as:
\[
  \begin{array}{lcl}
    \mathrm{carry}_0&=&\bot\\
...
\begin{RuleDescription}{bitblast_neg}
\begin{AletheX}
$i$. & \ctxsep & $\bvneg{x} ≈ A_1$ & \currule \\
\end{AletheX}
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (((\neg\bitof{0}\ x) \,\lsymb{xor}\,\bot)\,\lsymb{xor}\,\mathrm{carry}_0) \\
              & (((\neg\bitof{1}\ x) \,\lsymb{xor}\,\bot)\,\lsymb{xor}\,\mathrm{carry}_1) \\
              & \ldots \\
              & (((\neg\bitof{n - 1}\ x) \,\lsymb{xor}\, \bot)\,\lsymb{xor}\,\mathrm{carry}_{n-1})
\end{align*}
and, for $i\geq 0$:
\[
  \begin{array}{lcl}
    \mathrm{carry}_0&=&\top\\
    \mathrm{carry}_{i+1}&=&((\neg\bitof{i}\ x) \wedge \bot)
                            \vee 
                            (((\neg\bitof{i}\ x)\,\lsymb{xor}\, \bot)\wedge \mathrm{carry}_i)
  \end{array}
\]

\noindent
Alternatively, the rule may also be phrased as a ``short-circuiting'' of the
above when $x$ is a ``$\lsymb{bbT}$'' application. So given that:
\[
  \begin{array}{lcl}
    x&=&\bbterm\ x_0\ \ldots\ x_i\ \ldots \ x_j\ \ldots\ x_n\\
  \end{array}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvneg{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ ((\neg x_0 \,\lsymb{xor}\, \bot)\,\lsymb{xor}\,\mathrm{carry}_0)\ 
              \ldots\ ((\neg x_{n-1}\,\lsymb{xor}\, \bot)\,\lsymb{xor}\,\mathrm{carry}_{n-1})$, 
and $\mathrm{carry}_k$ being defined, for $k \geq 0$, as:
\[
  \begin{array}{lcl}
    \mathrm{carry}_0&=&\bot\\
    \mathrm{carry}_{i+1}&=&(\neg x_i\wedge \bot)\vee((\neg x_i\,\lsymb{xor}\, \bot) 
                            \wedge \mathrm{carry}_i)
  \end{array}
\]

\end{RuleDescription}

\begin{RuleDescription}{bitblast_mult}
\begin{AletheX}
$i$. & \ctxsep & $\bvmult{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
We perform bit-blasting following the \emph{shift add multiplier} way of computing.
That is, the term ``$A_1$'' is:
\begin{align*}
\bbterm\;& \mathrm{res}^{n-1}_0 \dots \mathrm{res}^{n-1}_{n-1}
\end{align*}
% TODO: fix this, F refers to:
...
in which, for $i,j\geq 0$,
      \[
        \begin{array}{lcl}
          ...
          \mathrm{res}^{j+1}_i&=&\mathrm{res}^j_i\, \lsymb{xor} \, 
                                \mathrm{sh}^{j+1}_i \, \lsymb{xor} \, \mathrm{carry}^{j+1}_i\\[1ex]
          \mathrm{carry}^0_i&=&\bot\\[1ex]
          \mathrm{carry}^{j+1}_{i+1}&=&\left\{
                                        \begin{tabular}{F{l}l}
                                          \mathrm{res}^j_i \wedge 
                                          \mathrm{sh}^{j+1}_i \vee
                                          ((\mathrm{res}^j_i \, \lsymb{xor} \, 
                                            \mathrm{sh}^{j+1}_i) \wedge 
                                            \mathrm{carry}^{j+1}_i)&if $j < i$\\
                                          \bot & otherwise\\
      ...
      \[
        \left[
          \begin{tabular}{F{l}F{c}F{l}}
            ...
            \mathrm{res}^3_1&=&\underbrace{(x_1\wedge y_0) \, \lsymb{xor} \,
                                (x_0\wedge y_1)} \, \lsymb{xor} \, 
                                \underbrace{\mathrm{carry}^3_1};\\
                            &&\qquad\,\underbrace{\mathrm{res}^0_1 \, \lsymb{xor} \,
                               \mathrm{sh}^1_1}\qquad\qquad\
                               \bot\\
                            &&\qquad\quad\ \,\mathrm{res}^2_1\\
            \mathrm{res}^3_2&=&\underbrace{(x_2\wedge y_0) \, \lsymb{xor} \,
                                (x_1\wedge y_1)} \, \lsymb{xor} \, 
                                \underbrace{\mathrm{carry}^1_2} \, 
                                \lsymb{xor} \, 
                                \underbrace{(x_0\wedge y_2)} \, \lsymb{xor} \,
                                \underbrace{\mathrm{carry}^3_2};\\
                            &&\qquad\,\underbrace{\mathrm{res}^0_2 \, 
                               \lsymb{xor} \,
                               \mathrm{sh}^1_2}\qquad\mathrm{res}^0_1
                               \wedge
                               \mathrm{sh}^1_1
                               \dots
                               \quad\mathrm{sh}^2_2\qquad\quad\bot\\
                            &&\qquad\qquad
                               \underbrace{\mathrm{res}^1_2\qquad\qquad\qquad\qquad}\\
                            &&\qquad\qquad\qquad\qquad\mathrm{res}^2_2\\
             \mathrm{res}^3_3&=&\underbrace{(x_3\wedge y_0) \, \lsymb{xor} \,
                                 (x_2\wedge y_1)} \, \lsymb{xor} \,
                                 \underbrace{\mathrm{carry}^1_3}
                                 \, \lsymb{xor} \,
                                 \underbrace{(x_1\wedge y_2)}
                                 \, \lsymb{xor} \,
                                 \underbrace{\mathrm{carry}^2_3}
                                 \, \lsymb{xor} \, 
                                 \underbrace{(x_0\wedge y_3)} \,
                                 \lsymb{xor} \, \underbrace{\mathrm{carry}^3_3};\\
                            &&\qquad\
                               \underbrace{\mathrm{res}^0_3 \, \lsymb{xor} \,
                               \mathrm{sh}^1_3\qquad\qquad
      ...
      \begin{tabular}{F{l}cF{l}l}
        \mathrm{carry}^1_3&=&\mathrm{res}^0_2 \wedge \mathrm{sh}^1_2 \vee 
                              ((\mathrm{res}^0_2 \, \lsymb{xor} \,
                              \mathrm{sh}^1_2) \wedge \mathrm{carry}^1_2)\\[1ex]
\mathrm{carry}^1_2&=&\mathrm{res}^0_1 \wedge \mathrm{sh}^1_1 \vee 
                      ((\mathrm{res}^0_1 \, \lsymb{xor} \,
                      \mathrm{sh}^1_1)\wedge\underbrace{\mathrm{carry}^1_1})\\
        &&\qquad\qquad\qquad\qquad\qquad\qquad\quad\!\bot\\[1ex]
\mathrm{carry}^2_3&=&\mathrm{res}^1_2\wedge\mathrm{sh}^2_2\vee
                      ((\mathrm{res}^1_2 \, \lsymb{xor} \, \mathrm{sh}^2_2)
                      \wedge\underbrace{\mathrm{carry}^2_2})\\
                          &&\qquad\qquad\qquad\qquad\qquad\qquad\quad\!\bot\\[1ex]
      \end{tabular}
\]
...
\begin{RuleDescription}{bitblast_and}
\begin{AletheX}
$i$. & \ctxsep & $\bvand{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x \,\wedge\, \bitof{0}\ y) \\
              & (\bitof{1}\ x \,\wedge\, \bitof{1}\ y) \\ \\
              & \ldots \\
              & (\bitof{n-1}\ x \,\wedge\, \bitof{n-1}\ y) \\
\end{align*}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvand{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ (x_0 \,\wedge\, y_0)\
             \ldots\  (x_{n-1} \,\wedge\, y_{n-1})$.
\end{RuleDescription}

\begin{RuleDescription}{bitblast_or}
\begin{AletheX}
$i$. & \ctxsep & $\bvor{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x \,\vee\, \bitof{0}\ y) \\
              & (\bitof{1}\ x \,\vee\, \bitof{1}\ y) \\ \\
              & \ldots \\
              & (\bitof{n-1}\ x \,\vee\, \bitof{n-1}\ y) \\
\end{align*}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvor{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ (x_0 \,\vee\, y_0)\
             \ldots\  (x_{n-1} \,\vee\, y_{n-1})$.
\end{RuleDescription}

\begin{RuleDescription}{bitblast_xor}
\begin{AletheX}
$i$. & \ctxsep & $\bvxor{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x \,\lsymb{xor}\, \bitof{0}\ y) \\
              & (\bitof{1}\ x \,\lsymb{xor}\, \bitof{1}\ y) \\ \\
              & \ldots \\
              & (\bitof{n-1}\ x \,\lsymb{xor}\, \bitof{n-1}\ y) \\
\end{align*}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvxor{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ (x_0 \,\lsymb{xor}\, y_0)\
             \ldots\  (x_{n-1} \,\lsymb{xor}\, y_{n-1})$.
\end{RuleDescription}

\begin{RuleDescription}{bitblast_xnor}
\begin{AletheX}
$i$. & \ctxsep & $\bvxnor{x}{y} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x \,≈\, \bitof{0}\ y) \\
              & (\bitof{1}\ x \,≈\, \bitof{1}\ y) \\ \\
              & \ldots \\
              & (\bitof{n-1}\ x \,≈\, \bitof{n-1}\ y) \\
\end{align*}
...
\begin{AletheX}
$i$. & \ctxsep & $\bvxnor{x}{y} ≈ A_2$ & \currule \\
\end{AletheX}
with $A_2 := \bbterm\ (x_0 \,≈\, y_0)\
             \ldots\  (x_{n-1} \,≈\, y_{n-1})$.
\end{RuleDescription}

\begin{RuleDescription}{bitblast_not}
\begin{AletheX}
$i$. & \ctxsep & $\bvnot{x} ≈ A_1$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n}.
The term ``$A_1$'' is
\begin{align*}
...
\begin{RuleDescription}{miniscope_distribute}
\begin{AletheX}
$i$. & \ctxsep & $ \forall x_1, \dots, x_n.(\varphi_1 \land \cdots \land \varphi_m) ≈
	(\forall x_1, \dots, x_n. \varphi_1) \land \cdots \land
  (\forall x_1, \dots, x_n. \varphi_m) $ & (\currule) \\
\end{AletheX}

or

\begin{AletheX}
$i$. & \ctxsep & $ \exists x_1, \dots, x_n.(\varphi_1 \lor \cdots \lor \varphi_m) ≈
	(\exists x_1, \dots, x_n. \varphi_1) \lor \cdots \lor
  (\exists x_1, \dots, x_n. \varphi_m) $ & (\currule) \\
\end{AletheX}
\end{RuleDescription}

\begin{RuleDescription}{miniscope_split}
\begin{AletheX}
$i$. & \ctxsep & $ \forall x_1, \dots, x_n.(\varphi_1 \lor \cdots \lor \varphi_m) ≈
  (\forall x_{1,1}, \dots, x_{1,n_1}. \varphi_1) \lor \cdots \lor
  (\forall x_{m,1}, \dots, x_{m,n_m}. \varphi_m) $ & (\currule) \\
\end{AletheX}

or

\begin{AletheX}
$i$. & \ctxsep & $ \exists x_1, \dots, x_n.(\varphi_1 \land \cdots \land \varphi_m) ≈
  (\exists x_{1,1}, \dots, x_{1,n_1}. \varphi_1) \land \cdots \land
  (\exists x_{m,1}, \dots, x_{m,n_m}. \varphi_m) $ & (\currule) \\
\end{AletheX}

where $\{x_{1,1}, \dots, x_{m,n_m}\}\subseteq\{x_1, \dots, x_n\}$
and the right side of the equality has no free variables in
$\{x_1, \dots, x_n\}$.

\end{RuleDescription}

\begin{RuleDescription}{miniscope_ite}
\begin{AletheX}
$i$. & \ctxsep & $
\forall x_1, \dots, x_n.(\lsymb{ite}\,\varphi_1\,\varphi_2\,\varphi_3) ≈
\lsymb{ite}\,\varphi_1\,(\forall x_1, \dots, x_n.\varphi_2)\,(\forall x_1, \dots, x_n.\varphi_3) $ & (\currule) \\
\end{AletheX}

where $\varphi_1$ has no free variable from $\{x_1, \dots, x_n\}$.
\end{RuleDescription}

\begin{RuleDescription}{aci_simp}
\begin{AletheX}
$i$. & \ctxsep & $ t_1 ≈ t_2 $ & (\currule) \\
\end{AletheX}

\noindent
where $t_1$ and $t_2$ are equivalent modulo associativity, commutativity, 
identity elements and idempotency. Checking this rule involves a normalization 
step of $t_1$ and $t_2$, that exploits the previous properties of the operations 
involved in both terms. The rule applies to SMT-LIB ACI operators (that is, 
those that satisfy associativity, commutativity, idempotency and that have an 
identity element), AC operators (associativity, commutativity and identity 
elements), as well to operators that satisfy only associativity and that have an 
identity element. The actual way in which the previous properties are exploited 
might be solver-dependent, which means that there is no such thing as an 
``ACI normal form'', standard among any solver producing Alethe certificates.

The operators that can trigger the previous process include:
\begin{itemize}
  \item Logical operations: \inlineAlethe{or}, \inlineAlethe{and}.
  \item Bit vectors: \inlineAlethe{bvor}, \inlineAlethe{bvand}, \inlineAlethe{bvadd}, 
                                   \inlineAlethe{bvmul}, \inlineAlethe{concat}.
  \item Arithmetic: \inlineAlethe{-} (binary substraction), \inlineAlethe{+}, \inlineAlethe{*}.
  \end{itemize}

The process itself can be described with a set of rewriting rules, which capture
a relation $\Rightarrow$ between terms representing the application of operators
and their simplified form. In what follows, $op$ will represent an operator with 
the appropriate property exploited in the rewriting rule being applied; $id$ will 
represent the $op$'s identity element (if appropriate); we will indicate with 
$x \not{\Rightarrow}$ that term $x$ cannot be further reduced through 
$\Rightarrow$ (which, in this case, reduces to check that $x$ is not an application 
of some ACI operator).
\begin{itemize}
\item $op \; x \; id \Rightarrow x$, for $x \not{\Rightarrow}$.
\item $op \; id \; x \Rightarrow x$, for $x \not{\Rightarrow}$.
\item $op \; x \; x \Rightarrow x$, for $x \not{\Rightarrow}$.
\item $op \; x_1 \; x_2 \Rightarrow op \; x_{1'} \; x_2$, where $x_1 \Rightarrow x_{1'}$.
\item $op \; x \; x_2 \Rightarrow op \; x \; x_{2'}$, where $x_2 \Rightarrow x_{2'}$ 
                                                           and 
                                                           $x \not{\Rightarrow}$.

\item $op \; x_1 \; x_2 \Rightarrow op \; x_{1'} \; x_{2'}$, 
            where 
            \begin{itemize}
            \item $x_1 \neq x_2$
            \item $x_{1'}, \; x_{2'} \in \{ x_1, x_2\}$
              
            \item Multi-sets $\{x_{1'}, x_{2'}\}$ and $\{x_1, x_2\}$ are equal.
              
              \item $x_1 \not{\Rightarrow}$ and $x_2 \not{\Rightarrow}$.
              
              % \item $x_{1'} \ge x_{2'}$, with $\ge$ being a total order among terms
              %   \textbf{TODO: is there something that we should ask for this order?}
            \end{itemize}
\end{itemize}

In the last rule, note that we do not impose some particular order among 
sub-terms. The intention of the rule is to allow for a particular implementation 
of a checker for the rule, to take advantage of the commutativity of $op$, if 
desired.

Also, note that no evaluation of the application of operators (\emph{constant folding})
is performed by the rule.

\end{RuleDescription}

\begin{RuleExample}
The following simple \proofRule{aci_simp} step, taken from a \textsf{QF\_UF} 
problem, serves to illustrate the application of a single reduction rule from 
$\Rightarrow$:
\begin{AletheVerb}
(step t1 (cl (= (and d d) d)) :rule aci_simp)
\end{AletheVerb}
The application of the rule exploits the idempotency property of
the \inlineAlethe{and} operator (third reduction rule from $\Rightarrow$), to 
conclude that \inlineAlethe{(= (and d d) d)} holds.
\end{RuleExample}

\begin{RuleExample}
The following example application of the \proofRule{aci_simp} rule illustrates 
a situation where the reduction of expressions, using $\Rightarrow$, also involves
\emph{aci-reduction} of sub-expressions:
\begin{AletheVerb}
(step t1 (cl (= (or false (or true true)) (or true false))) :rule aci_simp)
\end{AletheVerb}
As in the previous example, the application of the rule exploits an idempotency 
property, in this case of the \inlineAlethe{or} operator, but over the
sub-expression \inlineAlethe{(or true true)}. That is, the reduction also 
employs the fifth reduction rule defining $\Rightarrow$, that allows for the 
reduction of sub-expressions. Finally, in order to be able to conclude
that both terms are \emph{aci-compatible}, the reasoning employed exploits
the last reduction rule of $\Rightarrow$, that allows for an arbitrary order
of operands of applications of commutative operators. This allows us to conclude that
\inlineAlethe{(or false true)} (the result of the previous reduction step) is
aci-compatible with \inlineAlethe{(or true false)}. Note that, as explained
in the specification of $\Rightarrow$, there is no concrete order of
operands specified, leaving some degree of freedom for the implementation of
checkers of the rule. This, as long as the resulting $\Rightarrow$ relation ends up 
relating expressions that are equivalent modulo commutativity.
\end{RuleExample}

\begin{RuleDescription}{rare_rewrite}
\label{rule:rare_rewrite}
\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $\lsymb{rule}$) \\
\end{AletheX}

This rule refers to the application of the RARE 2.0 rewrite rule 
$\lsymb{rule}$, which should justify the conclusion $t ≈ s$. As described
in section \ref{sec:alethe:rare_rules}, a RARE rewrite rule is defined using a 
domain-specific language, called RARE. We assume that 
the producer of the Alethe proof certificate has at its disposal an arbitrary set of 
rules, written in RARE, for elaboration purposes of (some of) the equational 
reasoning steps in the proof. The rule name $\lsymb{rule}$
invoked in the previous rule must match with the name of some RARE rewrite rule 
in this set of rules. Also, the conclusion of the RARE rule invoked must
match with that of the application of rule \currule{} (that is, 
$ t ≈ s $).

The instantiation of the rewrite rule could require arbitrary arguments, with 
rule-dependent semantics. These arguments can be passed after the name of the 
invoked rewrite rule. For the purpose of providing these arguments, they must be 
wrapped into a list, marked with the $\lsymb{@list}$ terminal symbol. In this
case, the rule invocation would be as follows:

\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $\lsymb{rule}$, 
                                           ($\lsymb{@list} \; x_1, \dots, x_n)$)
\end{AletheX}

Where $x_1, \dots, x_n$, with $ n \geq 1$, are arbitrary arguments required by 
rule $\lsymb{rule}$. Note that the arguments are not required to have the same
type.

Finally, RARE rewrite rules can have premises, which can be also invoked when
applying a \currule{} step, as follows:

\begin{AletheXS}
$i_1$. & \ctxsep & $ t_1 ≈ s_1 $ & ($\dots$) \\
\aletheLineS
$i_n$. & \ctxsep & $ t_n ≈ s_n $ & ($\dots$) \\
$j$. & \ctxsep & $ t ≈ s $ & (\currule \; $i_1\; \dots i_n$)\; 
                             $\lsymb{rule} \; \dots$ \\
\end{AletheXS}

In this case, for the application of \currule{} to be valid, each premise must 
match with those of the rewrite rule referred, in the same order as invoked.
Also, the conclusion $ t ≈ s $ must match with that of the rewrite rule invoked.
It is possible to provide arbitrary arguments, as indicated in the previous 
case of \currule.

\end{RuleDescription}

\begin{RuleDescription}{multi_rare_rewrite}
\label{rule:multi_rare_rewrite}
\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $rule_1 \, \dots \, rule_n$) \\
\end{AletheX}
This rule refers to the repeated application of the RARE rewrite rules
$rule_1 \, \dots \, rule_n$ over the left-hand side of the conclusion, term
$t$, until it can be successfully rewritten as the right-hand side term $s$.
That is, among rules $rule_1 \, \dots \, rule_n$ there should be one rule
whose conclusion's left-hand side matches against term $t$. By applying
it, we should be able to rewrite term $t$ as some other term $t'$, such that it 
matches against the conclusion's left-hand side of some rule in
$rule_1 \, \dots \, rule_n$. This process should be repeated until some
intermediate term can be rewritten as term $s$ (that is, by applying a rewrite
rule whose conclusion's right-hand side matches $s$).


The instantiation of some of the referred rewrite rules could require arbitrary 
arguments, with rule-dependent semantics. These arguments can be passed after 
the name of the each invoked rewrite rule, using the list constructor
$\lsymb{@list}$, as in the case of rule \textsf{rare\_rewrite}
(rule \ref{rule:rare_rewrite}). In this
case, the rule invocation would be as follows:\\

\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $rule_1, 
                                           (\lsymb{@list} \; x_1, \dots, x_n),
                                           \dots,
                                           rule_n$)
\end{AletheX}

Where, after each rewrite rule that requires arguments, a list 
$\lsymb{@list}$ with the corresponding arguments is indicated. In the previous 
invocation of the \currule{} rule, we are assuming that $rule_1$ requires 
arguments, which are referred through the list 
$(\lsymb{@list} \; x_1, \dots, x_n)$. It is possible
to interleave with invocations of rules that do not require arguments.
In that case, only the rule name is mentioned. That would be the case of rule
$rule_n$ in our previous example.
