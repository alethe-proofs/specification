...
\begin{AletheX}
$i$. & \ctxsep & $\bvslt{x}{y} ≈ \mathrm{res}_{n-1}$ & \currule \\
\end{AletheX}
in which both $x$ and $y$ must have the same type \bitvec{n} and, for
$i\geq 0$:
\[
  \begin{array}{lcl}
    \mathrm{res}_0&=& \bitof{0}\ x \wedge \neg (\bitof{0}\ y)\\
    \mathrm{res}_{i+1}&=&((\bitof{i + 1}\ x ≈ \bitof{i + 1}\ y)\wedge \mathrm{res}_i)\vee
                       (\bitof{i+1}\ x \wedge \neg (\bitof{i+1}\ y))
  \end{array}
\]
...
The term ``$A_1$'' is
\begin{align*}
\bbterm\;& (\bitof{0}\ x \,\leftrightarrow\, \bitof{0}\ y) \\
              & (\bitof{1}\ x \,\leftrightarrow\, \bitof{1}\ y) \\ \\
              & \ldots \\
              & (\bitof{n-1}\ x \,\leftrightarrow\, \bitof{n-1}\ y) \\
\end{align*}
...
\end{AletheX}
with $A_2 := \bbterm\ (x_0 \,\leftrightarrow\, y_0)\
             \ldots\  (x_{n-1} \,\leftrightarrow\, y_{n-1})$.
\end{RuleDescription}
