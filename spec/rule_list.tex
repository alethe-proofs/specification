...
\begin{xltabular}{\linewidth}{l X}
    ...
    \ruleref{cp_normalize} & Steps for transforming an expressions to the normalized format. \\
    \ruleref{cp_literal} & Introduction of pseudo-boolean literal greater-equal to zero. \\
\end{xltabular}
...
\begin{RuleDescription}{cp_normalize}
    ...
    % The initial term is a relation of form:
    % \[\varphi_1 \bowtie \varphi_2\]
    % \noindent
    % Where $\varphi_1$ and $\varphi_2$ are sums of constants $k$ or linear terms $c\cdot l$
    % and $\bowtie \in \{=,>,<,\ge,\le\}$.

    % \noindent
    % Move variables to the left
    % \begin{itemize}
    %     \item $\varphi_1 \bowtie \varphi_2 \Rightarrow \varphi_1 - \varphi_2 \bowtie 0$
    % \end{itemize}

    % \noindent
    % Move constants to the right
    % \begin{itemize}
    %     \item $\varphi_1 + k_1 \bowtie k_2 \Rightarrow \varphi_1 \bowtie k_2 - k_1$
    % \end{itemize}

    % \noindent
    % Eliminate other relations
    % \begin{itemize}
    %     \item $\varphi = k \Rightarrow (\varphi \ge k) \land (\overline{\varphi} \ge -k)$
    %     \item $\varphi > k \Rightarrow \varphi \ge k +1$
    %     \item $\varphi < k \Rightarrow \overline\varphi \ge -k +1$
    %     \item $\varphi \le k \Rightarrow \overline\varphi \ge -k$
    % \end{itemize}
    ...
    % Push negations
    % \begin{itemize}
    %     \item $\overline\sum_i{c_i l_i} \ge k \Rightarrow \sum_i{-c_i l_i} \ge k$
    %     \item $\dots + -c_i l_i + \dots \ge k \Rightarrow \dots + c_i (1 - l_i) + \dots \ge k + c_i$
    % \end{itemize}
    ...
    % This rule transformas a linear relation term by applying equivalence-preserving
    % transformations until fixed point
    \noindent
    It has the form

    \begin{AletheXS}
        $i$. & \ctxsep  & $\Gamma$  & $(a \bowtie b) \approx (\sum_i{c_i l_i} \ge k) $ & \currule \\
    \end{AletheXS}

    where the right hand side is the normalized term and $\bowtie \in \{=,>,<,\ge,\le\}$.

    The possible transformations are:
    \begin{itemize}
        % Rethink this pattern matching
        % Move variables to the left
        \item $\varphi_1 \bowtie \varphi_2 \Rightarrow \varphi_1 - \varphi_2 \bowtie 0$
              % Move constants to the right
        \item $\varphi_1 \bowtie \varphi_2 \Rightarrow \varphi_1 - \varphi_2 \bowtie 0$
