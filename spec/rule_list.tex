...
\begin{AletheX}
$i$. & \ctxsep & $((\lsymb{extract}\ j\ i)\ x) ≈ (\bbterm\ \varphi_i\ \ldots\ \varphi_j)$ & (\currule) \\
\end{AletheX}

\noindent
where the formulas $\varphi_k$ are $(\bitof{k}\ x)$ for $i \leq k \leq j$.
...
\medskip
\begin{AletheX}
$i$. & \ctxsep & $((\lsymb{extract}\ j\ i)\ (\bbterm\ x_0\ \ldots\
...
\[
  \begin{array}{lcl}
    \mathrm{carry}_0&=&\bot\\
    \mathrm{carry}_{i+1}&=&((\bitof{i}\ x) \wedge (\bitof{i}\ y))\vee(((\bitof{i}\ x)\,\lsymb{xor}\, (\bitof{i}\ y))\wedge \mathrm{carry}_i)
  \end{array}
\]
...
\end{AletheX}
with $A_2 := (\bbterm\ (x_0 \,\lsymb{xor}\, y_0)\,\lsymb{xor}\,\mathrm{carry}_0\ \ldots\ (x_{n-1}
