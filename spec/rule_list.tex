...
and their simplified form. In what follows, $op$ will represent an operator with the appropriate properties for the rule being applied; $id$ will represent the $op$'s identity element (if appropriate); we will indicate with $x \not{\Rightarrow}$ that term $x$ cannot be further reduced through $\Rightarrow$.
\begin{itemize}
...
In the last rule, note that we do not impose some particular order among sub-terms. Also, note that no evaluation of the application of operators is performed by the rule.
