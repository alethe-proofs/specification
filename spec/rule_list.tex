...
step of $t_1$ and $t_2$, that exploits the previous properties of the operations involved 
in both terms. The rule applies to SMT-LIB ACI operators (that is, those that satisfy
associativity, commutativity, idempotency and that have an identity element), 
AC operators (associativity, commutativity and identity elements), as well to
operators that satisfy only associativity and that have an identity element.
The actual way in which the previous properties are exploited might be solver-dependent, 
which means that there is no such thing as an ``ACI normal form'', standard among
any solver producing Alethe certificates.
...
\item $op \; x \; x_2 \Rightarrow op \; x \; x_{2'}$, where $x_2 \Rightarrow x_{2'}$ and $x \not{\Rightarrow}$.
...
In the last rule, note that we do not impose some particular order among sub-terms. 
The intention of the rule is to allow for a particular implementation, of a checker for the
rule, to take advantage of the commutativity of $op$, if desired.
