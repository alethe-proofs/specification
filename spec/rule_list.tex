...
\begin{RuleDescription}{pbblast_bvconcat}
    ...
    The `concat' operation over two BitVectors $\textbf{s}$ with $n$ bits and
    $\textbf{t}$ with $m$ bits, that returns a BitVector with $n+m$ bits
    is expressed using PseudoBoolean inequalities by:

    \begin{AletheS}
        $i$. & \ctxsep & \, & $(\lsymb{concat}\ \textbf{s}\ \textbf{t}) \approx
            \lsymb{pbbterm}\; c_0 \dots c_{n+m-1} $ & (\currule)
    \end{AletheS}
...
\begin{RuleDescription}{pbblast_bvequality}
    % TODO
    WIP
\end{RuleDescription}
