...
\begin{RuleDescription}{pbblast_bvand_ith_bit}
    This rule extracts assertions of the $i$th bit of an application of \proofRule{pbblast_bvand}, given
    its arguments were $x_i$ and $y_i$:
    \begin{AletheX}
        $i$. & \ctxsep & $(\textbf{x}_i\ge r_i) \land (\textbf{y}_i\ge r_i) \land (r_i + 1 \ge\textbf{x}_i + \textbf{y}_i)$ & (\currule) \\
    \end{AletheX}

    \noindent
    In which $r_i$ is the element from the pseudo boolean bit blasting of the \textbf{bvand} rule of form:

    \[\varepsilon z.\, (\textbf{x}_i\ge z) \land (\textbf{y}_i\ge z) \land (z + 1 \ge\textbf{x}_i + \textbf{y}_i)\]
