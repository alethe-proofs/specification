...
Every operator with the previous properties can trigger the normalization step.
In particular:
\begin{itemize}
...
The normalization process itself can be described with these simple rewriting rules:
\begin{itemize}
\item For an operator $op$ with identity element $id$, and operands $x_i$:
\begin{itemize}
\item $op \; x_1 \; id ⇒ x_1$
\item $op \; id \; x_2 ⇒ x_2$
\end{itemize}
\item For an idempotent operators $op$, and operand $x$:
\begin{itemize}
\item $op \; x \; x ⇒ x$
\end{itemize}

\item Optionally, commutativity might be taken into consideration, rewriting
          terms as in:
          \begin{itemize}
          \item $op \; x_1 \; x_2 ⇒ op \; x_{1'} \; x_{2'}$, 
            where 
            \begin{itemize}
