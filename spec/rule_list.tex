...
\begin{RuleDescription}{pbblast_bvadd}
    ...
    \noindent
    Ensuring that each $c_i$ element  for $i = 0, \dots, n+\left\lceil{\log_2 N}\right\rceil-1$ satisfies:

    \[
        \sum_{i=0}^{n+\left\lceil{\log_2 N}\right\rceil - 1} {2^i c_i}
        = \sum_{j=1}^{N}{\sum_{i=0}^{n-1}{2^i \textbf{a}_{i}^{j}}}
    \]
\end{RuleDescription}

\begin{RuleDescription}{pbblast_bvmult}
    The `bvmul' operation over $N$ BitVectors $\textbf{a}^j$ for $j = 1, \dots, N$
    with $n$ bits that returns a BitVector of $n\cdot N$ bits is expressed
    using PseudoBoolean inequalities by:

    \begin{AletheS}
        $i$. & \ctxsep & \, & $(\lsymb{bvmul}\ \textbf{a}^1\ \dots\ \textbf{a}^N) \approx
            \lsymb{pbbterm}\; c_0 \dots\ c_{n\cdot N} $ & (\currule)
    \end{AletheS}

    \noindent
    Ensuring that each $c_i$ element  for $i = 0, \dots, n\cdot N$ satisfies:

    \[
        \sum_{k_1} \cdots \sum_{k_n} {
        2^{\sum_{l=1}^{N}{k_l}}
        \bigwedge_{l=1}^{N}{\textbf{a}_{k_l}^{l}}
        }
        = \sum_{i=0}^{n\cdot N}{2^i c_i}
    \]

    \noindent
    In which the $\land$ operator between pseudo-boolean values actually means
    the constraints:
...
\begin{RuleDescription}{pbblast_bvsign_extend}
    The `sign\_extend' operation over the non-negative integer $k$ and
    a BitVector $\textbf{x}$ with $n$ bits, will return a BitVector of $n+k$
    bits is equal to $x$ on the lowest bits and repeats the most valuable bit
    of $x$ $k$ times on the highest bits. This is expressed using PseudoBoolean
    inequalities by:

    \begin{AletheS}
        $i$. & \ctxsep & \, & $((\lsymb{sign\_extend}\ k)\ \textbf{x}) \approx
            \lsymb{pbbterm}\; c_0 \dots c_{n+k-1} $ & (\currule)
    \end{AletheS}

    \noindent
    Each term ``$c_i$'' is a pseudo boolean equality on the ranging over
    $0 \le i < n + k$, such that if $i < n$:
...
\begin{RuleDescription}{pbblast_bvashr}
    % TODO https://smt-lib.org/logics-all.shtml#QF_BV
