...
\aletheLineS
$i_m$. & \ctxsep  & $\Gamma$  & $t_m ≈ u_m $ & ($\dots$) \\
$j$. & \ctxsep  & $\Gamma$  & $(f\,s_1\,\cdots\,s_n\, t_1\,\cdots\,t_m) ≈
...
\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $\lsymb{rewrite\_rule}$) \\
\end{AletheX}

This rule refers to the application of the RARE 2.0 rewrite rule 
$\lsymb{rewrite\_rule}$, which should justify the conclusion $t ≈ s$. As described
...
reasoning steps in the proof. The rule name $\lsymb{rewrite\_rule}$
...
rule $\lsymb{rewrite\_rule}$. Note that the arguments are not required to have the same
type.
