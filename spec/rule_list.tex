...
\begin{AletheX}
$i$. & \ctxsep & $ t ≈ s $ & (\currule \; $rewrite\_rule$) \\
\end{AletheX}

This rule refers to the application of the RARE rewrite rule 
$rewrite\_rule$, which should justify the conclusion $t ≈ s$. Recall that 
...
reasoning steps in the proof. The rule name $rewrite\_rule$ 
...
rule $rewrite\_rule$. Note that the arguments are not required to have the same
type.
