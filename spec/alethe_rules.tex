...
$i$. & \ctxsep   & 
$l_1$, \dots, $l_k$, \dots, $l_m$, \dots, $l_n$  & (\dots) \\
$j$. & \ctxsep   & 
$\top$  & (\currule\; $i$) \\
\end{AletheX}
...
\begin{AletheX}
$i_1$. & [leftframe=on] \ctxsep  & $\varphi_1$  & $\proofRule{assume}$ \\
\aletheLineB
$i_n$. & [leftframe=on] \ctxsep  & $\varphi_n$  & $\proofRule{assume}$ \\ 
\aletheLineB
\bTR[bottomframe=on]\bTD[bottomframe=off] $j$. & [leftframe=on] \ctxsep  & $\psi$  & (\dots) \\ 
...
$i$. & \ctxsep     & 
$\varphi_1$, \dots , $\varphi_o$  & \currule\, [$a_1$, \dots, $a_o$] \\
\end{AletheX}
...
$i$. & \ctxsep     & 
$\varphi_1$, \dots , $\varphi_o$  & (\currule) \\
\end{AletheX}
...
\aletheLineS
\bTR[bottomframe=on] \bTD[bottomframe=off] $j$. & [leftframe=on]
...
\ruleparagraph{Remark.} In \in{Section}[sec:qsimp-proof] we discuss an alternative
...
\ruleparagraph{Remark.} While the connective could be $≈$,
