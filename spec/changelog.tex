...
Proof rules:
\begin{itemize}
  \item General rewriting of rule \proofRule{la_generic}, mainly focused on presentation. The algorithm stays the same.
  \item Added rules \proofRule{rare_rewrite} and \proofRule{multi_rare_rewrite}, which serves as an interface with rewriting rules written in the RARE language.
  \item Added rule \proofRule{aci_simp}, that describe ACI reasoning performed 
            to simplify terms using SMT-LIB ACI operators.
  \item Rule \proofRule{connective_def} extended to complement the case
            where an existential quantification is rewritten as a negated universal
            quantification.
  ...
  \item Addition of miniscoping rules: \proofRule{miniscope_split}, \proofRule{miniscope_distribute}, \proofRule{miniscope_ite}
  \item Addition of the \proofRule{shuffle} to permute the arguments of a
        commutative operator.
  \item Clarify that \proofRule{distinct_elim} cannot be applied to only one
        argument due to SMT-LIB restrictions.
\end{itemize}
...
\begin{itemize}
  ...
  \item Restrict sorting of numbers such that the sort of a constant is
        only determined by its syntactic category.
  \item Make \proofRule{cong} more flexible.  It is now possible to skip
        a prefix of the arguments if they are already the same on both sides.
        This is intended to future-proof the format for support of partial
        application.
\end{itemize}
...
\begin{itemize}
  ...
  \item Highlight the equality used with context by using the $\ceq$ symbol.
  \item Specify that printing a proof in SMT-LIB mode requires the printing
        of extra parentheses around the proof.
  \item Clarify the handling of variadic operators.
\end{itemize}
